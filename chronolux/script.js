...
let wishlist = loadStorage(storageKeys.wishlist, []);
let productModal;
...
function productCard(product, index) {
  ...
  return `
    <div class="col-md-6 col-xl-4 product-column" data-aos="fade-up" data-aos-delay="${(index % 3) * 70}">
      <article class="product-card tilt-card" data-id="${product.id}">
...
function initializeParticles() {
  ...
  for (let index = 0; index < 35; index += 1) {
    ...
    particle.style.bottom = `${-10 - Math.random() * 30}px`;
    particle.style.animationDuration = `${8 + Math.random() * 13}s`;
...
function initializeTypewriter() {
  ...
  const phrases = ["visionaries.", "collectors.", "the exceptional.", "generations."];
  let phraseIndex = 0;
...
document.getElementById("checkoutButton").addEventListener("click", () => {
  ...
  Swal.fire({
    icon: "success",
    title: "Thank you",
    text: "Thank you for shopping with ChronoLux.",
